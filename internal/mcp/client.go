...
func NewClient(cfg models.MCPServerConfig) (*Client, error) {
	var clientTransport transport.Interface
	switch cfg.Type {
	case "sse":
		if sseTransport, err := transport.NewSSE(cfg.URL); err != nil {
			return nil, err
		} else {
			clientTransport = sseTransport
		}
	case "http":
		if httpTransport, err := transport.NewStreamableHTTP(cfg.URL); err != nil {
			return nil, err
		} else {
	...
	client := mcpClient.NewClient(clientTransport)
...
func (c *Client) ListTools(context context.Context, request mcp.ListToolsRequest) (*[]mcp.Tool, error) {
...
func (c *Client) ListPrompts(context context.Context, request mcp.ListPromptsRequest) (*[]mcp.Prompt, error) {
...
func (c *Client) ListResources(context context.Context, request mcp.ListResourcesRequest) (*[]mcp.Resource, error) {
